...
public class ShaderUtil {
        ...
        try {
            ...
            switch (fragmentShaderLoc) {
                ...
                case "roundedRectGradient":
                    ...
                    break;
                default:
                    fragmentShaderID = createShader(net.minecraft.client.Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation(fragmentShaderLoc)).getInputStream(), GL_FRAGMENT_SHADER);
                    break;
            }
            glAttachShader(program, fragmentShaderID);

            int vertexShaderID = createShader(net.minecraft.client.Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation(vertexShaderLoc)).getInputStream(), GL_VERTEX_SHADER);
    ...
    public ShaderUtil(String fragmentShadersrc, boolean notUsed) {
        ...
        try {
            vertexShaderID = createShader(net.minecraft.client.Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("Tenacity/Shaders/vertex.vsh")).getInputStream(), GL_VERTEX_SHADER);
        } catch (IOException e) {
    ...
    public void setUniformMatrix4(String name, FloatBuffer matrix) {
        glUniformMatrix4(getUniform(name), false, matrix);
    }
    ...
    public static void drawQuads() {
        ScaledResolution sr = TRenderUtil.getScaledResolution();
