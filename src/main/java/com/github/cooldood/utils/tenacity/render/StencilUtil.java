...
public class StencilUtil {
    ...
    private static int lastStencilBufferID = -1;

    public static void checkSetupFBO(Framebuffer framebuffer) {
        if (framebuffer != null) {
            // Only setup if it's a new depth buffer (not our custom one)
            if (framebuffer.depthBuffer > -1 && framebuffer.depthBuffer != lastStencilBufferID) {
    ...
    public static void setupFBO(Framebuffer framebuffer) {
        ...
        framebuffer.depthBuffer = stencilDepthBufferID;
        lastStencilBufferID = stencilDepthBufferID;
    }
